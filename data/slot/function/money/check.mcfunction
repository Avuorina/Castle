...
## やるぞ。
    ...
    execute if score $AFMoney _ matches 1000..9000 unless score $Money _ matches 1000..9000 run data modify entity @n[tag=money_importer_display] item.components."minecraft:custom_model_data".strings set value ["only"]
