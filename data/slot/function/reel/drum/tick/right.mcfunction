...
## 加速中：速度を上げ（tick数を減らし）、巡航速度に達したら定速へ
    ...
    execute if score @s ReelDrumState_R matches 1 if score @s ReelDrumSpeed_R matches ..2 run scoreboard players set @s ReelDrumState_R 2
...
## 減速中：目標面に来ていれば着地確定、まだなら速度を落としていく
    ...
    execute if score @s ReelDrumState_R matches 3 if score @s ReelDrumSpeed_R matches 5.. run scoreboard players set @s ReelDrumSpeed_R 5
