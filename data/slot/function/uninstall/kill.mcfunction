...
# 周辺のリール（item_display）とレバー（interaction）をkill
    ...
    kill @e[type=item_display,tag=slot_button_display,limit=3,sort=nearest]
    kill @n[type=text_display,tag=slot_medal]
    kill @n[type=text_display,tag=info_display]
    kill @n[type=interaction,tag=slot_lazy]
    kill @n[type=text_display,tag=point_display]
    kill @n[type=text_display,tag=plus_point_display]
    kill @n[type=item_display,tag=Stage]
    kill @n[type=text_display,tag=StageDisplay]
