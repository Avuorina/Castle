...
# 周辺のリール（item_display）とレバー（interaction）をkill
    ...
    execute at @s run kill @e[type=interaction,tag=slot_lazy,distance=..3]
    execute at @s run kill @e[type=text_display,tag=point_display,distance=..3]
    execute at @s run kill @e[type=text_display,tag=plus_point_display,distance=..3]
